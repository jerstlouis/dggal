...
C(Module) theEcereModule;

#ifdef ECERE_STATIC
unsigned int __ecereDll_Load_ecere(C(Module) * module);
unsigned int __ecereDll_Unload_ecere(C(Module) * module);
#endif

LIB_EXPORT C(Application) eC_init(C(Module) fromModule, C(bool) loadEcere, C(bool) guiApp, int argc, char * argv[])
{
#ifdef _DEBUG
   // printf("%s_init\n", "eC");
#endif
...
#ifdef ECERE_STATIC
      C(Module) module = Module_loadStatic(fromModule, loadEcere ? "ecere" : "ecereCOM", true, (void *)(__ecereDll_Load_ecere), (void *)(__ecereDll_Unload_ecere));
#else
